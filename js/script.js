...
const displayNewsDitails = (categoryDitails) => {
    ...
    if(categoryDitails.length == 0){
        document.getElementById('not-found').innerHTML =`<h3 class="bg-success bg-opacity-25 rounded ps-4 py-3" >Not Found</h3>`;
    }
    else{
        document.getElementById('not-found').innerHTML =`<h3 class="bg-success bg-opacity-25 rounded ps-4 py-3">${categoryDitails.length} Items Found</h3>`;
